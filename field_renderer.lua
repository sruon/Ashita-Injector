...
local field_utils = require('field_utils');
local field_data = require('field_data');
...
function field_renderer.render_numeric_array_input(field, value_table)
    local changed = false;
    imgui.Text(('Array[%d]:'):fmt(field_utils.safe_get_array_size(field)));
...
function field_renderer.render_field_label(field)
    imgui.TextColored({ 0.4, 0.8, 1.0, 1.0 }, field_utils.safe_get_name(field));
    imgui.SameLine();
    imgui.TextColored({ 0.4, 1.0, 0.4, 1.0 }, ('(%s)'):fmt(field_utils.safe_get_type(field)));

    if field_utils.should_show_bits_info(field) then
        imgui.SameLine();
        imgui.TextColored({ 0.7, 0.7, 0.7, 1.0 }, (' (%d bits)'):fmt(field_utils.safe_get_bits(field)));
    end
end
...
function field_renderer.render_complete_field(field, data, hex_displays)
    ...
    if field_utils.is_char_array(field) then
        -- Char arrays get right-aligned input
    ...
    elseif field_utils.is_numeric_array(field) then
        -- Numeric arrays don't need special layout
        field_renderer.render_field_input(field, data, hex_displays);
    else
        -- Scalar fields get helper buttons and hex toggles
        ...
        if imgui.TreeNode(('##struct_%s_%d'):fmt(field.name, struct_idx), ('[%d]'):fmt(struct_idx)) then
            ...
            for _, nested_field in ipairs(field.nested_fields) do
                if not nested_field.name:find('padding') then
                    -- Normalize the nested field to match regular field structure
                    local normalized_field = field_utils.normalize_nested_field(nested_field);

                    -- Create unique hex display key for nested fields
                    local nested_field_name = ('%s_%d_%s'):fmt(field.name, struct_idx, nested_field.name);
                    local nested_hex_displays = {};
                    nested_hex_displays[normalized_field.name] = hex_displays[nested_field_name];

                    -- Reuse the exact same rendering logic as regular fields
                    field_renderer.render_complete_field(normalized_field, struct_data[nested_field.name],
                        nested_hex_displays);

                    -- Copy back the hex display state
                    hex_displays[nested_field_name] = nested_hex_displays[normalized_field.name];
                end
            end
